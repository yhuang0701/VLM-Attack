...
class Attacker:
    ...
    def attack_constrained(self, text_prompt, img, batch_size = 8, num_iter=2000, alpha=1/255, epsilon = 128/255 ):
        ...
        for t in tqdm(range(num_iter + 1)):
            ...
            text_prompts=[]
            flag=False
            for idx, b in enumerate(batch_targets):
                if len(b)<2:
                    flag=True
                    break
                text_prompts.append(text_prompt.format(prompt=b[0]))
            if flag:
                print(f"error in iter {t}, batch_targets: {batch_targets}")
                continue
            # print(batch_targets)
            batch_targets = [b[1] for b in batch_targets]
            # print(batch_targets)
            # print(text_prompts)
            ...
            print("target_loss, target_loss, target_loss, target_loss: %f" % (
                target_loss.item())
                  )
